...
//sliders
    let r = board.create('slider', [[1, 3], [3, 3], [1, 4, 3]], {name: 'R', snapWidth: 0.25, visible: true});
    let changeR = (newR) => {
        console.log("changing R: " + newR)
        r.setValue(newR);
    }
    if(localStorage.getItem('r-input') != null){
        changeR(localStorage.getItem('r-input'));
    } else localStorage.setItem('r-input', r.Value());

    document.getElementById('current-r').innerText = (String)(r.Value());
    document.getElementById('main-form:r-input').value = r.Value();
    let R = () => r.Value();
    let minusR = () => -r.Value();
...
//quarter corner (quarter circle)
    ...
    r.on('mousedrag', () => {
        document.getElementById('current-r').innerText = (String)(r.Value());
        document.getElementById('main-form:r-input').value = r.Value();
        localStorage.setItem('r-input', r.Value());
    })
    ...
    document.getElementById('box1').addEventListener('mousedown', (e, i) => {
        ...
        document.getElementById("hidden-form:r_hidden_chooser").value = r.Value();
